...
# todo: how about not super slow
with_matches = []

for x in data.iterrows():
    row = x[1]
    fuzzy_match = process.extractOne(row.shelf, shelf_options)

    if fuzzy_match[1] >= 90:
        with_matches.append({"shelf": fuzzy_match[0], "Title": row.Title, "Author": row.Author})
    else:
        continue


st.write(pd.DataFrame(columns=["shelf", "Title", "Author"], data=with_matches))
